  ...
  return (
    <div className="mt-8 space-y-4">
      ...
      <p className="text-xs text-[#7a5c4e]">
        Times below are in your base timezone
        {sitterTz ? <>: <strong>{sitterTz}</strong></> : " (set base city on dashboard for correct local times)"}.
        Customer local time is also shown when available.
      </p>
      {bookings.map((b) => {
          ...
          <article key={b.id} className="rounded-2xl border border-[#e8d5c4] bg-[#fff8f0]/90 p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                ...
                {customer?.city ? (
                  <p className="text-xs text-[#7a5c4e]">
                    Service location: {customer.city}
              ...
              {slots.map((s) => (
                <li key={s.id || s.starts_at} className="rounded-lg border border-[#e8d5c4]/80 bg-white px-3 py-2">
                  <div>
                    ...
                    {formatInTimezone(s.starts_at, sitterTz)} → {formatInTimezone(s.ends_at, sitterTz)}
                    {sitterTz ? <span className="text-xs text-[#7a5c4e]"> ({sitterTz})</span> : null}
                  </div>
                  {customerTz && customerTz !== sitterTz ? (
                    <div className="mt-0.5 text-xs text-[#7a5c4e]">
                      Customer time: {formatInTimezone(s.starts_at, customerTz)} →{" "}
                      {formatInTimezone(s.ends_at, customerTz)} ({customerTz})
                    </div>
                  ) : null}
                </li>
              ))}
