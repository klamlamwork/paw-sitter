...
export async function POST(request) {
  try {
    ...
    const { data: post } = await admin.from("shop_kol_posts").select("id, author_profile_id, source_type, status, verified_order_item_id, primary_product_id").eq("id", postId).maybeSingle();
    if (!post || post.author_profile_id !== profile.id || post.source_type !== "verified_purchase" || post.status !== "draft" || post.verified_order_item_id !== itemId || post.primary_product_id !== access.item.product_id) return NextResponse.json({ error: "Verified media draft not found." }, { status: 404 });
    ...
    const { data: revision, error: revisionErr } = await admin.from("shop_kol_post_revisions").insert({ post_id: post.id, revision_number: Number(lastRevision?.revision_number || 0) + 1, title, body: text, rating, content_type: "review", moderation_status: "pending", moderation_reasons: [], submitted_at: now }).select("id").single();
    if (revisionErr) throw revisionErr;
    const { error: productErr } = await admin.from("shop_kol_post_products").upsert({ post_id: post.id, product_id: access.item.product_id, is_primary: true }, { onConflict: "post_id,product_id" });
    if (productErr) throw productErr;
    const { data: tags } = await admin.from("shop_product_tags").select("tag_id").eq("product_id", access.item.product_id);
    if ((tags || []).length) {
    ...
    const { error: postErr } = await admin.from("shop_kol_posts").update({ pending_revision_id: revision.id, status: "pending_admin", updated_at: now }).eq("id", post.id).eq("status", "draft");
