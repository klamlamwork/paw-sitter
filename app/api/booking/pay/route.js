...
export async function POST(request) {
  try {
    ...
    if (!booking_id || !["card", "etransfer", "later"].includes(payment_method)) return NextResponse.json({ error: "Invalid payment request." }, { status: 400 });

    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return NextResponse.json({ error: "Sign in to pay." }, { status: 401 });

    const { data: booking } = await supabase.from("bookings").select("id, status, payment_status, sitter_id, service_type, estimated_total, booking_slots(starts_at)").eq("id", booking_id).eq("customer_id", user.id).single();
    if (!booking || booking.status !== "accepted" || !booking.estimated_total) return NextResponse.json({ error: "Booking not ready for payment." }, { status: 400 });
    const hours = hoursUntilUTC((booking.booking_slots || [])[0]?.starts_at);
    if (hours !== null && hours < 48) return NextResponse.json({ error: "Payment must be made at least 48 hours before the booking starts." }, { status: 400 });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { user_id: user.id, booking_id: String(booking_id), sitter_id: String(booking.sitter_id), service_type: booking.service_type || "" },
      line_items: [{ quantity: 1, price_data: { currency: "cad", unit_amount: totalCents, product_data: { name: booking.service_type === "house_sit" ? "House sit" : "Sitter booking" } } }],
    });
    const { error: stampErr } = await supabase.from("bookings").update({ payment_method: "card", payment_status: "pending", updated_at: new Date().toISOString() }).eq("id", booking_id);
