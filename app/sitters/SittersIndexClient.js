...
export default function SittersIndexClient({ sitters = [], loadError = "" }) {
  ...
  const filtered = useMemo(() => {
    ...
    return (sitters || []).filter((s) => {
      ...
      if (services.length) {
        ...
        // OR: sitter offers at least one selected service
    ...
    return (
      <p className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
        ...
        <span className="mt-1 block text-xs">
          If this mentions policy/permission, run sql/15-public-sitters-read.sql in Supabase.
        </span>
      </p>
    );
  }
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 rounded-2xl border border-[#e8d5c4] bg-white p-4 sm:flex-row sm:flex-wrap sm:items-end">
        <label className="block min-w-[180px] flex-1 text-xs font-semibold text-[#7a5c4e]">
          ...
          <input
            ...
            className="mt-1 w-full rounded-xl border border-[#e8d5c4] px-3 py-2 text-sm font-normal text-[#3b2a22]"
          />
        </label>
        <label className="block min-w-[160px] text-xs font-semibold text-[#7a5c4e]">
          ...
          <select
            ...
            className="mt-1 w-full rounded-xl border border-[#e8d5c4] bg-white px-3 py-2 text-sm font-normal text-[#3b2a22]"
          >
      ...
      <p className="mt-4 text-xs text-[#7a5c4e]">
              ...
              >
                <div className="aspect-[16/9] w-full overflow-hidden bg-[#fff1e6]">
                  {s.profile_pic_url ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center text-sm font-medium text-[#c4a484]">
                ...
                <div className="p-5">
                  ...
                  {s.bio ? (
                    <p className="mt-3 line-clamp-3 text-sm text-[#5c4033]">{s.bio}</p>
                  ) : (
                    <p className="mt-3 text-sm text-[#7a5c4e]">No bio yet.</p>
                  )}
                  {svcs.length ? (
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {svcs.map((svc) => (
                        <span
                          ...
                          className="rounded-full border border-[#e8d5c4] bg-[#fff8f0] px-2 py-0.5 text-[11px] font-semibold text-[#5c4033]"
                        >
