...
export default function RatingOptionsClient({ categories = [], initialOptions = [] }) {
  const [options, setOptions] = useState(initialOptions);
  const [form, setForm] = useState({ category_id: categories[0]?.id || "", label: "", description: "", sort_order: "0", file: null });
  ...
  async function addOption(e) {
    ...
    try {
      ...
      if (form.file) {
        ...
        const path = `${form.category_id}/${Date.now()}.${ext}`;
    ...
    <div className="mt-6 space-y-6">
      <form onSubmit={addOption} className="space-y-3 rounded-2xl border border-[#e8d5c4] bg-white p-4">
        ...
        <label className="block text-sm">Category
          <select className="mt-1 w-full border border-[#e8d5c4] px-3 py-2" value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })}>
        ...
        <button type="submit" disabled={busy || !form.category_id} className="rounded-full bg-[#c45c26] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{busy ? "Saving…" : "Add option"}</button>
      </form>
      <ul className="space-y-2">
        {options.map((o) => (
          <li key={o.id} className="flex items-center justify-between gap-3 rounded-xl border border-[#e8d5c4] bg-white px-3 py-2 text-sm">
            <span className="flex items-center gap-2">{o.icon_url ? <img src={o.icon_url} alt="" className="h-6 w-6 object-contain" /> : null}<span><strong>{o.label}</strong> · {catName[o.category_id] || "Category"}{o.description ? <span className="block text-xs text-[#7a5c4e]">{o.description}</span> : null}</span></span>
