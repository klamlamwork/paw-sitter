...
export default function BrandsAdminClient({ initialBrands }) {
  ...
  async function toggleFeatured(b) {
    ...
    setBrands((list) =>
      list.map((x) => (x.id === b.id ? { ...x, is_featured: !x.is_featured } : x))
    );
  }
  ...
  return (
    <div className="mt-8 space-y-8">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={
                  ...
                  (b.is_featured ? "bg-[#c45c26] text-white" : "border border-[#e8d5c4]")
                }
              >
      ...
      {!brands.length ? <p className="text-sm text-[#7a5c4e]">No brands yet.</p> : null}
    </div>
  );
}
