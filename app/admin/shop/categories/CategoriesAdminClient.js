...
export default function CategoriesAdminClient({ initialCategories }) {
  ...
  async function addCat(e) {
    ...
    setCats((list) => [...list, data].sort((a, b) => a.sort_order - b.sort_order || a.name.localeCompare(b.name)));
  ...
  return (
    <div className="mt-8 space-y-8">
      ...
      <ul className="space-y-2">
        {cats.map((c) => (
          <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[#e8d5c4] bg-white px-4 py-3 text-sm">
            <div>
              <p className="font-semibold">{c.name}</p>
              <p className="text-xs text-[#7a5c4e]">/shop/c/{c.slug} · sort {c.sort_order}</p>
            </div>
