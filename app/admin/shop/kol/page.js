...
export default async function AdminKolQueuePage() {
  ...
  const [authorsResult, productsResult, revisionsResult, mediaResult, eventsResult] = await Promise.all([
    ...
    productIds.length ? admin.from("shop_products").select("id, name").in("id", productIds) : { data: [] },
    revisionIds.length ? admin.from("shop_kol_post_revisions").select("id, title, body, rating, submitted_at").in("id", revisionIds) : { data: [] },
    list.length ? admin.from("shop_kol_post_media").select("id, post_id, public_id, version, resource_type, sort_order").in("post_id", list.map((post) => post.id)).eq("lifecycle", "attached_private").order("sort_order") : { data: [] },
    list.length ? admin.from("kol_moderation_events").select("id, post_id, stage, decision, reasons").in("post_id", list.map((post) => post.id)).order("created_at", { ascending: false }) : { data: [] },
  ]);
  ...
  const eventsByPost = {};
  for (const row of eventsResult.data || []) (eventsByPost[row.post_id] ||= []).push(row);
