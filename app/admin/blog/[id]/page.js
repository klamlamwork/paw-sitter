...
export default async function AdminBlogEditPage({ params }) {
  ...
  if (!post) notFound();
  const [{ data: tags }, { data: posts }, { data: pt }, { data: pp }, { data: pr }] = await Promise.all([
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8">
        <BlogPostEditor
          ...
          allProducts={[]}
          allPosts={posts || []}
          initialTagIds={(pt || []).map((r) => r.tag_id)}
          initialProductIds={(pp || []).map((r) => r.product_id)}
          initialRelatedIds={(pr || []).map((r) => r.related_post_id)}
        />
      </div>
    </div>
  );
