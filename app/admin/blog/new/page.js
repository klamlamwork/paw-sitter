...
export default async function AdminBlogNewPage() {
  ...
  const [{ data: tags }, { data: products }, { data: posts }] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8">
        <BlogPostEditor mode="create" authorId={profile.id} allTags={tags || []} allProducts={(products || []).filter((p) => p.is_active !== false)} allPosts={posts || []} />
      </div>
    </div>
  );
}
