...
export default async function SellerOrdersPage() {
  ...
  const { data: orders } = await supabase
    .from("shop_orders")
    .select("id, status, payment_status, payment_method, discount_cents, discount_code, shipping_cents, shipping_label, shipping_method, created_at, shipping_name, shipping_phone, shipping_email, shipping_line1, shipping_line2, shipping_city, shipping_state, shipping_postal_code, shipping_country, shop:shop_shops!seller_shop_id(id, name), items:shop_order_items(id, qty, price_cents, currency, refund_status, product:shop_products(name, slug))")
