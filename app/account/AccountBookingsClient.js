              ...
              <li key={b.id} className="rounded-2xl border border-[#e8d5c4] bg-[#fff8f0]/90 px-4 py-3 text-sm">
                <div className="flex justify-between gap-2">
                  <span className="font-semibold">
                    {serviceLabel(b.service_type)} - {b.sitters?.display_name || "Sitter"}
                  </span>
                  ...
                  <div className="mt-2">
                    {!canPay ? (
                      ...
                      <>
                        ...
                        >
                          {open ? "Hide payment options" : "Pay now"}
                        </button>
                        {open && (
                          <div className="mt-2 space-y-2 rounded-xl border border-[#e8d5c4] bg-white p-3">
                            <p className="text-xs font-medium text-[#3b2a22]">How will you pay?</p>
                            <button
                              type="button"
                              disabled={busyId === b.id}
                              onClick={() => choosePayment(b.id, "card")}
                              className="block w-full rounded-lg border border-[#e8d5c4] px-3 py-2 text-left text-xs font-semibold disabled:opacity-60"
                            >
                              {busyId === b.id ? "Working…" : "Pay with card (Stripe)"}
                            </button>
                            <button
                              type="button"
                              disabled={busyId === b.id}
                              onClick={() => choosePayment(b.id, "etransfer")}
                              className="block w-full rounded-lg border border-[#e8d5c4] px-3 py-2 text-left text-xs font-semibold disabled:opacity-60"
                            >
                              Interac e-Transfer (seller confirms when received)
                            </button>
                            <button
                              type="button"
                              disabled={busyId === b.id}
                              onClick={() => choosePayment(b.id, "later")}
                              className="block w-full rounded-lg border border-[#e8d5c4] px-3 py-2 text-left text-xs font-semibold disabled:opacity-60"
                            >
                              Pay later
                            </button>
                ...
                {canCancel && (
                  <div className="mt-2">
                    ...
                    <button
                      ...
                      className="rounded-full border border-[#e8d5c4] bg-white px-4 py-1.5 text-xs font-semibold disabled:opacity-60"
                    >
                      Cancel booking
                    </button>
                  </div>
