      ...
      <ul className="mt-4 space-y-3">
        {(bookings || []).length === 0 ? (
          ...
          bookings.map((b) => {
            ...
            const hoursUntilStart = startsAt ? (new Date(startsAt).getTime() - Date.now()) / (1000 * 60 * 60) : null;
            ...
            return (
              <li key={b.id} className="rounded-2xl border border-[#e8d5c4] bg-[#fff8f0]/90 px-4 py-3 text-sm">
                ...
                {startsAt && (
                  <p className="mt-1 text-xs text-[#7a5c4e]">Starts: {new Date(startsAt).toLocaleString()}</p>
                )}
                {showPay && (
                  <div className="mt-2">
                    {!canPay ? (
