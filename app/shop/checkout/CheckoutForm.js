...
export default function CheckoutForm({ userId, cartId, items = [], subtotalCents = 0, defaultAddress, hasSavedAddress }) {
  ...
  useEffect(() => {
    ...
    const initial = {};
    for (const [shopId] of byShop.entries()) {
  ...
  useEffect(() => {
    ...
    async function runQuote() {
      if (!items?.length || !form.line1 || !form.city || !form.state || !form.country) {
        setShippingQuotes([]);
      ...
      try {
        ...
        if (!cancelled) {
          setShippingQuotes([]);
          setTotalShippingCents(0);
          setError(err.message || "Could not quote shipping");
        }
      } finally {
    ...
    try {
      ...
      if (form.payment_method === "card") {
        const res = await fetch("/api/shop/checkout", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ address: form, promo_code: promoCode?.code || null, shipping_selections: shopMethods }),
        });
        const data = await res.json();
  ...
  const total = Math.max(0, (subtotalCents || 0) - discount + totalShippingCents);
  const allPickup = Object.values(shopMethods).every((m) => m === "pickup");
  ...
  return (
    <form onSubmit={submit} className="mt-4 space-y-4">
          ...
          <ul className="mt-2 space-y-1">
            {items.map((item) => (
              <li key={item.id} className="flex items-center justify-between gap-3">
                <span>{item.product?.name || "Shop item"} × {item.qty || 1}</span>
      ...
      <div className="rounded-2xl border border-[#e8d5c4] p-3">
        ...
        {quoting ? <p className="mt-2 text-xs text-[#7a5c4e]">Calculating…</p> : null}
            ...
            return (
              <div key={shopId} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#e8d5c4] p-2 text-sm">
                ...
                <select
                  ...
                  value={method}
                  onChange={(e) => setShopMethods((m) => ({ ...m, [shopId]: e.target.value }))}
                  disabled={blocked}
                >
      ...
      {!allPickup ? (
        ...
        <p className="text-xs text-[#7a5c4e]">Pickup selected for all shops — shipping address hidden. Billing details still required for payment.</p>
      )}
