...
export default async function RateProductPage({ params }) {
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-10 sm:px-6">
      ...
      {!access.ok ? <p className="mt-4 text-sm text-red-600">{access.reason}</p> : <RateProductForm itemId={itemId} options={access.options} hasExistingReview={!!access.existingReview} />}
    </div>
  );
}
