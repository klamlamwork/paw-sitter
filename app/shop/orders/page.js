...
export default async function ShopOrdersPage({ searchParams }) {
  ...
  const { data: orders } = await supabase
    .from("shop_orders")
    .select("id, status, created_at, shipping_city, seller_shop_id, shop:shop_shops!seller_shop_id(id, name, slug), items:shop_order_items(id, qty, price_cents, currency, product_id, product:shop_products(name, slug))")
          ...
          {orders.map((order) => {
            ...
            return (
              <li key={order.id} className="rounded-2xl border border-[#e8d5c4] bg-white p-4">
                ...
                <p className="mt-3 text-sm font-bold text-[#3b2a22]">Total {formatShopPrice(total, currency)}</p>
              </li>
            );
          })}
        </ul>
