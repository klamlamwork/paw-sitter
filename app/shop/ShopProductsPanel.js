...
function ProductCard({ product, coverUrl }) {
  ...
  return (
    <Link href={productPath(product)} className="group flex h-full min-h-[7.5rem] w-full overflow-hidden rounded-2xl border border-[#e8d5c4] bg-white transition hover:border-[#c45c26]/50 sm:min-h-0 sm:flex-col">
      <div className="relative h-28 w-28 shrink-0 bg-[#fff8f0] sm:aspect-square sm:h-auto sm:w-full sm:shrink-0">
        {coverUrl ? <img src={coverUrl} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 flex items-center justify-center text-xs text-[#7a5c4e]">No image</div>}
      </div>
      <div className="flex min-h-0 min-w-0 flex-1 flex-col px-3 py-2.5 sm:py-3">
        <span className="line-clamp-2 min-h-[2.5rem] text-sm font-semibold leading-snug text-[#3b2a22] group-hover:text-[#c45c26]">{product.name}</span>
        <span className="mt-1 line-clamp-2 min-h-[2rem] text-xs text-[#7a5c4e]">{product.short_description || "\u00a0"}</span>
        <span className="mt-auto pt-1.5 text-sm font-semibold text-[#c45c26]">{price || "\u00a0"}</span>
      </div>
    </Link>
  );
}
...
function ToggleChip({ active, onClick, children }) {
  return (
    <button type="button" onClick={onClick} aria-pressed={active} className={"shrink-0 px-3 py-1 text-xs font-semibold transition " + (active ? "bg-[#c45c26] text-white" : "border border-[#e8d5c4] bg-white text-[#5c4033]")}>
      {children}
    </button>
  );
}
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedLon.size > 0) {
      list = list.filter((p) => {
        ...
        for (const lon of selectedLon) if (!labels.has(String(lon).toLowerCase())) return false;
        return true;
      });
    }
    if (sort === "price_asc") {
    ...
    <div className="mt-4 space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(categoriesRow1 || []).map((c) => (
              <ToggleChip key={c.id} active={selectedCats.has(c.id)} onClick={() => setCats(toggleInSet(selectedCats, c.id))}>{c.name}</ToggleChip>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {longevityLabels.map((label) => (
              <ToggleChip key={label} active={selectedLon.has(label)} onClick={() => setLon(toggleInSet(selectedLon, label))}>{label}</ToggleChip>
            ))}
          </div>
        </div>
      ) : null}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <label className="flex items-center gap-2 text-xs font-semibold text-[#5c4033]">
          Sort by
          <select className="border border-[#e8d5c4] bg-white px-3 py-1.5 text-xs font-semibold" value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}>
      ...
      {totalPages > 1 ? (
        <nav className="flex flex-wrap items-center justify-center gap-2 pt-2" aria-label="Pagination">
          <button type="button" disabled={safePage <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="border border-[#e8d5c4] px-3 py-1 text-xs font-semibold disabled:opacity-40">Previous</button>
          {Array.from({ length: totalPages }).map((_, i) => {
            const n = i + 1;
            return (
              <button key={n} type="button" onClick={() => setPage(n)} className={"flex h-8 w-8 items-center justify-center text-xs font-semibold " + (n === safePage ? "bg-[#c45c26] text-white" : "border border-[#e8d5c4] text-[#5c4033]")}>
                {n}
              </button>
            );
          })}
          <button type="button" disabled={safePage >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="border border-[#e8d5c4] px-3 py-1 text-xs font-semibold disabled:opacity-40">Next</button>
        </nav>
      ) : null}
    </div>
  );
