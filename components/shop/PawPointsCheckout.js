...
export default function PawPointsCheckout({ orderCents = 0, items = [], onChange }) {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      const res = await fetch("/api/paw-points/quote", {
        ...
        body: JSON.stringify({ order_cents: orderCents, points: want, items }),
      });
  ...
  return (
    <div className="rounded-2xl border border-[#e8d5c4] p-3 text-sm">
      ...
      {quote?.earn_points ? <p className="mt-1 text-xs text-green-700">Earn {quote.earn_points} points on the cash portion of this order (after fulfillment).</p> : null}
