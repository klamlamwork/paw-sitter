...
export default function AuthNav({ profile }) {
  ...
  if (profile.role === "admin") {
    links.push({ href: "/admin/sitters", label: "Admin sitters" });
    links.push({ href: "/admin/blog", label: "Admin blog" });
  }
  ...
  return (
    <div className="relative flex items-center gap-2" ref={rootRef}>
      <div className="hidden items-center gap-3 sm:flex">
        {links.map((l) => (<Link key={l.href + l.label} href={l.href} className={deskLinkClass}>{l.label}</Link>))}
      ...
      <div className="flex items-center sm:hidden">
        <button type="button" aria-expanded={open} aria-label="More menu" onClick={() => setOpen((v) => !v)}
        ...
        {open ? (
          <div className="absolute right-0 top-full z-50 mt-2 w-60 rounded-2xl border border-[#e8d5c4] bg-white p-2 shadow-lg">
            {links.map((l) => (<Link key={l.href + l.label} href={l.href} className={menuItemClass} onClick={() => setOpen(false)}>{l.label}</Link>))}
