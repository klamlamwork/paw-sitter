...
function ProfileAvatar({ profile }) {
  ...
  return (
    <span className="inline-flex h-9 w-9 items-center justify-center overflow-hidden rounded-full border border-[#e8d5c4] bg-[#f3e0d0] text-sm font-bold text-[#c45c26]">
    ...
    <div>
      ...
      {open ? (
        <div className="mb-1 ml-2 border-l border-[#e8d5c4] pl-2">
          {items.map((l) => (
            <Link key={l.href} href={l.href} className="block rounded-lg px-3 py-2 text-sm font-medium text-[#5c4033] hover:bg-[#fff8f0]" onClick={onNavigate}>
              {l.label}
            </Link>
          ))}
        </div>
    ...
    <div className="relative">
      ...
      {open ? (
        <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-2xl border border-[#e8d5c4] bg-white p-2 shadow-lg">
          {items.map((l) => (
            <Link key={l.href} href={l.href} className="block rounded-lg px-3 py-2.5 text-sm font-medium text-[#5c4033] hover:bg-[#fff8f0]" onClick={() => setOpen(false)}>
              {l.label}
            </Link>
          ))}
        </div>
  ...
  return (
    <div className="relative flex items-center gap-2" ref={rootRef}>
      <div className="hidden items-center gap-3 sm:flex">
        {isSitter ? (
          <DeskMenu title="Sitter account" items={sitterLinks} open={desk === "sitter"} setOpen={() => setDesk((d) => (d === "sitter" ? "" : "sitter"))} />
        ) : null}
        ...
        <DeskMenu
          ...
          trigger={<ProfileAvatar profile={profile} />}
        />
        <form action="/auth/signout" method="post">
          <button type="submit" className="rounded-full border border-[#e8d5c4] bg-white px-3 py-1.5 text-xs font-semibold text-[#5c4033]">Log out</button>
        </form>
      </div>

      <div className="flex items-center sm:hidden">
        <button type="button" aria-expanded={open} aria-label="More menu" onClick={() => setOpen((v) => !v)} className="mr-1">
          <ProfileAvatar profile={profile} />
        </button>
        <button type="button" aria-expanded={open} aria-label="More menu" onClick={() => setOpen((v) => !v)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#e8d5c4] bg-white text-[#5c4033] shadow-sm active:bg-[#fff8f0]">
          <MoreIcon />
        </button>
        {open ? (
          <div className="absolute right-0 top-full z-50 mt-2 w-64 rounded-2xl border border-[#e8d5c4] bg-white p-2 shadow-lg">
            ...
            <form action="/auth/signout" method="post" className="mt-1 border-t border-[#e8d5c4] pt-1">
              <button type="submit" className="w-full rounded-lg px-3 py-2.5 text-left text-sm font-semibold text-[#c45c26] hover:bg-[#fff8f0]">Log out</button>
            </form>
          </div>
        ) : null}
      </div>
