...
as $$
  select
    greatest(coalesce(sum(delta) filter (where status = 'available'), 0), 0)::int as available,
    greatest(coalesce(sum(delta) filter (where status = 'pending'), 0), 0)::int as pending,
    greatest(coalesce(-sum(delta) filter (where status = 'reserved' and delta < 0), 0), 0)::int as reserved
