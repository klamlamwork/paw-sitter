-- Products with no variety still need stock.
-- Also recreates set_shop_order_fulfillment (fixes schema-cache miss).
...
alter table public.shop_order_stock_moves
...
drop function if exists public.set_shop_order_fulfillment(uuid, text);
...
begin
  ...
  if o.status = p_new_status then return; end if;
  ...
  if p_new_status = 'accepted' then
    ...
    if not exists (select 1 from public.shop_order_stock_moves where order_id = o.id) then
      for item in select * from public.shop_order_items where order_id = o.id
      loop
        remaining := item.qty;
        ...
        if item.variant_id is not null then
          ...
          select exists (
            select 1 from public.shop_product_batches b where b.variant_id = item.variant_id
          ) into has_batches;
          ...
          if has_batches then
            ...
            loop
              ...
              update public.shop_product_batches
                set qty_on_hand = qty_on_hand - take,
                    ...
                    updated_at = now()
                where id = rec.id;
              insert into public.shop_order_stock_moves (order_id, order_item_id, product_id, variant_id, batch_id, qty)
              values (o.id, item.id, item.product_id, item.variant_id, rec.id, take);
            ...
            update public.shop_product_variants
              set stock_qty = stock_qty - remaining, updated_at = now()
              where id = v.id;
            insert into public.shop_order_stock_moves (order_id, order_item_id, product_id, variant_id, batch_id, qty)
            values (o.id, item.id, item.product_id, item.variant_id, null, remaining);
            remaining := 0;
          else
            remaining := 0;
          end if;
        else
          select * into p from public.shop_products where id = item.product_id for update;
          if not found then continue; end if;
          if coalesce(p.track_stock, true) then
            if coalesce(p.stock_qty, 0) < remaining then
            ...
            update public.shop_products
              ...
              where id = p.id;
            insert into public.shop_order_stock_moves (order_id, order_item_id, product_id, variant_id, batch_id, qty)
            values (o.id, item.id, item.product_id, null, null, remaining);
          end if;
          remaining := 0;
        end if;
    ...
    update public.shop_orders set status = 'accepted', updated_at = now() where id = o.id;
    return;
  end if;
  ...
  if p_new_status in ('declined', 'cancelled') then
    ...
    if o.status = 'accepted' then
      for mv in select * from public.shop_order_stock_moves where order_id = o.id
      loop
        if mv.batch_id is not null then
          update public.shop_product_batches
            set qty_on_hand = qty_on_hand + mv.qty,
                status = case when status = 'depleted' and qty_on_hand + mv.qty > 0 then 'active' else status end,
                updated_at = now()
            where id = mv.batch_id;
        elsif mv.variant_id is not null then
          update public.shop_product_variants
            set stock_qty = stock_qty + mv.qty, updated_at = now()
            where id = mv.variant_id;
        elsif mv.product_id is not null then
          update public.shop_products
            set stock_qty = stock_qty + mv.qty, updated_at = now()
    ...
    update public.shop_orders set status = p_new_status, updated_at = now() where id = o.id;
    return;
  end if;
  ...
  if p_new_status = 'shipped' then
    ...
    update public.shop_orders set status = 'shipped', updated_at = now() where id = o.id;
  end if;
end;
$$;
