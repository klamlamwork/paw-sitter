...
async function hydratePublishedPosts(admin, list, productById = {}) {
  ...
  const [authorsResult, revisionsResult, mediaResult, productsResult] = await Promise.all([
    ...
    postIds.length ? admin.from("shop_kol_post_media").select("id, post_id, public_id, version, resource_type, sort_order, lifecycle").in("post_id", postIds).in("lifecycle", ["published", "attached_private"]).order("sort_order") : { data: [] },
