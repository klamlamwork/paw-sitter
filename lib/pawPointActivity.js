...
export function groupPawPointActivity(rows = []) {
  ...
  for (const row of rows || []) {
    ...
    if (EARN_REASONS.has(row.reason) && delta > 0) {
      // A fulfilled pending lot creates an available row with lot_id pointing to
      // the original pending row. Treat both as one customer-facing earn.
  ...
  const out = [
    ...
    ...Array.from(earns.values()).map((row) => {
      ...
      return {
        ...row,
        points,
        label: `${reasonLabel(row.reason)}${refFor(row) ? ` · ${refFor(row)}` : ""}`,
