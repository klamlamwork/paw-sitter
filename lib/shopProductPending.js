import { normalizeProductType, defaultInventoryMode } from "@/lib/shopInventory";
...
export function buildProductSnapshot(product, media = [], longevityItems = [], categoryIds = []) {
  ...
  return {
    ...
    inventory_mode: product.inventory_mode || defaultInventoryMode(productType),
...
export function snapshotFromForm(form, media, longevityItems, categoryIds = []) {
  ...
  return {
    ...
    product_type: productType,
    inventory_mode: form.inventory_mode || defaultInventoryMode(productType),
...
export async function applySnapshotToProduct(supabase, productId, snap) {
  ...
  const productType = normalizeProductType(snap.product_type);
  const patch = {
    name: snap.name,
    slug: snap.slug,
    ...
    category_id: cats[0] || null,
    product_type: productType,
    inventory_mode: snap.inventory_mode || defaultInventoryMode(productType),
    status: "approved",
