...
export async function canRateItem(itemId, profile) {
  ...
  const selfReview = await ownsAnyShop(admin, profile.id, [
    item.order?.seller_shop_id,
    item.product?.primary_shop_id,
    item.product?.brand_shop_id,
  ]);
...
export async function submitProductReview({ profile, itemId, rating, title, body, optionIds }) {
  ...
  const ticks = (optionIds || [])
    .filter((id) => allowed.has(id))
    .map((option_id) => ({ review_id: review.id, option_id }));
  if (ticks.length) {
