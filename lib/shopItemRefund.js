...
export async function applyShopItemRefund({ orderItemId, sellerUserId }) {
  ...
  const { data: order } = await admin
    .from("shop_orders")
    .select("id, shop_id, user_id, payment_status, stripe_session_id, stripe_payment_intent, refund_status")
    .eq("id", item.order_id)
    .maybeSingle();
  if (!order) throw new Error("Order not found.");

  const { data: shop } = await admin.from("shop_shops").select("id, owner_profile_id").eq("id", order.shop_id).maybeSingle();
  ...
  await admin.from("shop_orders").update({
    ...
    payment_status: allRefunded ? "refunded" : "partially_refunded",
    refunded_at: allRefunded ? now : null,
    updated_at: now,
  }).eq("id", order.id);
