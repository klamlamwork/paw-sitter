...
const CA_POSTAL_PREFIX = {
  A: "NL", B: "NS", C: "PE", E: "NB", G: "QC", H: "QC", J: "QC",
  K: "ON", L: "ON", M: "ON", N: "ON", P: "ON", R: "MB", S: "SK", T: "AB", V: "BC", X: "NT", Y: "YT",
};
...
export function inferProvinceFromPostal(postal, country) {
  const code = countryCode(country || "CA");
  const compact = String(postal || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
  if (code === "CA" && compact.length >= 1) return CA_POSTAL_PREFIX[compact[0]] || "";
  return "";
}

export function normalizeRegionCode(country, provinceState) {
  const raw = clean(provinceState);
  if (!raw) return null;
  if (/^[A-Z]{2}-[A-Z]{2}$/.test(raw)) return raw;
  const c = countryCode(country || "CA");
  const code = c === "CA" ? CA_PROVINCES[raw] : c === "US" ? US_STATES[raw] : raw;
...
export function isRegionExcluded(regionCode, excludeRegions) {
  ...
  return excludeRegions.some((value) => {
    const rule = clean(value);
    return rule === regionCode || (rule.endsWith("-*") && regionCode.startsWith(rule.slice(0, -1)));
  });
}
...
export function buyerAddressFields(address = {}) {
  ...
  const provinceState =
    ...
    address.state ||
    address.province ||
    inferProvinceFromPostal(address.postal_code, country) ||
    "";
  return { country, provinceState };
}

function listedRegions(list, country) {
  return (list || []).map((value) => normalizeRegionCode(country, value) || clean(value)).filter(Boolean);
}
...
export function resolveShippingScope({
  ...
  const national = listedRegions(nationalRegions, "Canada");
  if (national.length && !national.includes(region)) {
...
export function resolveShippingRate({ settings, method, scope, subtotalCents }) {
  const src = settings || defaultShippingSettings();
  const m = method === "express" || method === "pickup" ? method : "standard";
  if (m === "express" && src.express_enabled === false) return null;
  if (m === "pickup" && src.pickup_enabled === false) return null;
  const s = scope === "national" || scope === "us" ? scope : "home";
  const freeOver = Number(src[`${m}_${s}_free_over_cents`]);
  if (Number.isFinite(freeOver) && freeOver > 0 && subtotalCents >= freeOver) {
    return {
      cents: 0,
      minDays: src[`${m}_${s}_min_days`] ?? null,
      maxDays: src[`${m}_${s}_max_days`] ?? null,
      label: labelFor(m, s, src, true),
    };
  }
...
function labelFor(method, scope, settings, free) {
  ...
  return `${name}${min != null && max != null ? ` · ${min}–${max} business days` : ""}`;
}
