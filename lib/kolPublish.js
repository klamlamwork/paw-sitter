...
export async function publishKolPost({ postId, adminProfileId }) {
  ...
  const { data: post } = await admin
    .from("shop_kol_posts")
    .select("id, author_profile_id, source_type, status, pending_revision_id, verified_order_item_id, primary_product_id, reward_source_key")
    .eq("id", postId)
    .maybeSingle();
  ...
  if (points > 0) {
    await grantKolPendingReward({
      post: publishedPost,
      userId: post.author_profile_id,
      points,
  ...
  const { error: revisionErr } = await admin
    ...
    .update({ moderation_status: "approved_admin", reviewed_at: now, reviewed_by: adminProfileId })
  ...
  const { error: postErr } = await admin
    ...
    .update({
      ...
      verified_badge: sourceType === "verified_purchase",
      published_revision_id: post.pending_revision_id,
  ...
  await admin.from("kol_moderation_events").insert({
    ...
    stage: "admin_review",
    decision: "published",
    reasons: [sourceType === "verified_purchase" ? "verified_purchase" : "community"],
  });
  return { ok: true, status: "published", source_type: sourceType, reward_points: points, reward_available_at: availableAt };
}
