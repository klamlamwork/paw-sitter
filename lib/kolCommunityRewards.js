...
export async function releaseCommunityKolReward(postId) {
  ...
  if (!already) {
    await appendLedger({ user_id: pending.user_id, delta: pending.delta, status: "available", reason: "earn_kol", source_key: `${pending.source_key}:available`, lot_id: pending.id, order_id: pending.order_id || null, expires_at: pending.expires_at, remark: "Paw Points earned · Review - Community" });
