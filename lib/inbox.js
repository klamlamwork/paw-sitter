...
export async function ensureInboxForUser(profile) {
  ...
  let query = admin.from("bookings").select("id, customer_id, sitter_id, customer_message, created_at");
  query = sitter?.id
  ...
  for (const booking of list) {
    ...
    const preview = previewText(booking.customer_message) || "New booking request";
