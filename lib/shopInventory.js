...
/** True if a batch can still be sold on the storefront */
...
/** Sum sellable qty for one variant from its batches (expired lots excluded) */
export function sellableQtyFromBatches(batches) {
  return (batches || []).reduce((sum, b) => sum + (isSellableBatch(b) ? b.qty_on_hand || 0 : 0), 0);
}
