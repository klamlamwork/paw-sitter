...
export async function reconcileVerifiedKolForRefundedItem(orderItemId) {
  ...
  const { error: reviewErr } = await admin.from("shop_product_reviews").update({ verified_purchase: false }).eq("order_item_id", item.id);
  if (reviewErr && !/verified_purchase/i.test(reviewErr.message || "")) throw reviewErr;
