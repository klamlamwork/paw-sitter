...
export const SERVICE_TYPES = {
  house_sit: { id: "house_sit", label: "House sit", rateUnit: "night", description: "Overnight stay at the pet's home.", schedule: "range", petsOptional: true },
  drop_in: { id: "drop_in", label: "Drop-in visit", rateUnit: "30 min", description: "Short visits for care at the pet's home.", schedule: "visits", petsOptional: true },
  walking: { id: "walking", label: "Dog / cat walking", rateUnit: "30 min", description: "Walks for dogs and/or cats.", schedule: "visits", petsRequired: true },
  boarding: { id: "boarding", label: "Boarding", rateUnit: "night", description: "Pets stay overnight at the sitter's place.", schedule: "range", petsRequired: true },
};
...
export function getEffectiveWindow(sitterId, dateObj, weeklyRows, overrideRows, timeZone, serviceType) {
  ...
  const rows = (weeklyRows || []).filter((r) => r.sitter_id === sitterId && r.day_of_week === dayNum);
...
export function serviceAvailableOnDay(sitterId, serviceType, dateObj, dayAvailability, timeZone) {
  ...
  return {
    hasOverride: true, available: true,
...
export function filterAvailableSitters({
  ...
  for (const sitter of sitters) {
    ...
    results.push({
      ...
      _radiusKm: unlimited ? null : Number(svc.radius_km),
