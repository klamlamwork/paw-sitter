...
export async function grantVerifiedTextReviewPoints({ userId, reviewId, orderId, availableAt }) {
  ...
  const pending = await appendLedger({ user_id: userId, delta: qty, status: "pending", reason: "earn_kol", source_key: sourceKey, order_id: orderId || null, remark: "Paw Points earned · Review - Verified purchase", expires_at: addDays(new Date().toISOString(), 365), available_at: availableAt || null });
  if (!availableAt || new Date(availableAt) <= new Date()) await releaseReviewLot(pending);
  return pending;
}
...
async function releaseReviewLot(pending) {
  ...
  if (already) return;
  await appendLedger({ user_id: pending.user_id, delta: pending.delta, status: "available", reason: "earn_kol", source_key: `${pending.source_key}:available`, lot_id: pending.id, order_id: pending.order_id || null, expires_at: pending.expires_at, remark: "Paw Points earned · Review - Verified purchase" });
...
export async function releaseDueVerifiedTextReviewRewards() {
  ...
  const { data: rows } = await admin.from("paw_point_ledger").select("id, user_id, delta, source_key, order_id, expires_at, available_at").eq("reason", "earn_kol").eq("status", "pending").gt("delta", 0).like("source_key", "shop_review:%");
  const released = [];
