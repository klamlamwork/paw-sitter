...
function shopReleaseAt(order) {
  ...
  if (order?.delivered_at) {
    ...
    if (!Number.isNaN(d.getTime())) return new Date(d.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString();
  }
  return new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
}

async function shopEscrowParts(admin, order, items, shopPct) {
  try {
    const { data: settlement } = await admin
      .from("shop_order_settlements")
      .select("merchandise_cents, shipping_cents, discount_cents, discount_sponsor, points_redeemed_cents, seller_escrow_cents, platform_escrow_cents, display_total_cents")
      .eq("order_id", order.id)
      .maybeSingle();
    if (settlement && (settlement.seller_escrow_cents != null || settlement.platform_escrow_cents != null)) {
      const gross = Math.max(0, Number(settlement.merchandise_cents) || 0) + Math.max(0, Number(settlement.shipping_cents) || 0);
      return {
        gross_cents: gross,
        commission_pct: shopPct,
        commission_cents: Math.max(0, Number(settlement.platform_escrow_cents) || 0),
        discount_cents: Math.max(0, Number(settlement.discount_cents) || 0),
        discount_funded_by: settlement.discount_sponsor === "platform" ? "platform" : settlement.discount_sponsor === "vendor" ? "vendor" : null,
        platform_absorbed_cents: settlement.discount_sponsor === "platform" ? Math.max(0, Number(settlement.discount_cents) || 0) + Math.max(0, Number(settlement.points_redeemed_cents) || 0) : Math.max(0, Number(settlement.points_redeemed_cents) || 0),
        net_cents: Math.max(0, Number(settlement.seller_escrow_cents) || 0),
        from_settlement: true,
      };
    }
  } catch {
    /* settlements table optional */
  }
  const gross = (items || []).reduce((sum, i) => sum + (i.price_cents || 0) * (i.qty || 0), 0);
  const discountCents = Math.max(0, Number(order.discount_cents) || 0);
  const fundedBy = order.discount_funded_by === "platform" ? "platform" : (discountCents ? "vendor" : "vendor");
  return applyEscrowDiscount({ grossCents: gross, commissionPct: shopPct, discountCents, fundedBy });
}
...
export async function syncEscrowFromPaidActivity() {
  ...
  const { data: orders, error: orderErr } = await admin
    .from("shop_orders")
    .select("id, seller_shop_id, status, payment_status, discount_cents, discount_funded_by, stripe_payment_intent, delivered_at, return_window_ends_at")
  ...
  for (const o of orders || []) {
    ...
    const parts = await shopEscrowParts(admin, o, items, cfg.shop_commission_pct);
    const gross = parts.gross_cents || 0;
    ...
    if (!existing) {
      ...
      const { error: insertErr } = await admin.from("escrow_entries").insert({
        ...
        notes: parts.from_settlement
          ? "Held after buyer paid. Releases 7 days after delivery."
          : "Held after buyer paid. Releases 7 days after delivery.",
      });
