...
export async function syncEscrowFromPaidActivity() {
  ...
  const { data: bookings, error: bookingErr } = await admin
    .from("bookings")
    .select("id, sitter_id, status, payment_status, payment_received, estimated_total, stripe_payment_intent")
  ...
  const { data: orders, error: orderErr } = await admin
    .from("shop_orders")
    .select("id, seller_shop_id, status, payment_status, stripe_payment_intent")
