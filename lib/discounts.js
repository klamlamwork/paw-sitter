...
export function computeDiscountCents(code, subtotalCents, shippingCents = 0) {
  ...
  if (code.type === "percent") {
    return Math.max(0, Math.round((subtotal * (Number(code.percent_off) || 0)) / 100));
  }
  if (code.type === "fixed" || code.type === "threshold") {
    return Math.max(0, Math.min(subtotal, Number(code.fixed_off_cents) || 0));
  }
  if (code.type === "shipping" || code.applies_to_shipping) {
    return Math.max(0, Math.min(shipping, Number(code.fixed_off_cents) || shipping));
  }
  return 0;
}
...
export function quoteDiscount(code, items = [], { shippingCents = 0 } = {}) {
  ...
  let rows = [...byVendor.values()];
  // Site-wide: if no vendor ids, attribute across all items
  if (code.scope === "site" && !rows.length && items.length) {
    const totalGross = items.reduce((sum, item) => sum + itemTotal(item), 0) || 1;
    ...
    for (const item of items) {
      ...
      if (!vendorId) continue;
      const key = `${vendorType}:${vendorId}`;
      const current = tempMap.get(key) || { vendorType, vendorId, gross: 0, discount: 0 };
      current.gross += itemTotal(item);
  ...
  return {
    ...
    debug: { eligibleCount: eligible.length, itemsCount: items.length, eligibleSubtotal, discountCents },
  };
}
...
export async function quoteShopCode(raw, userId, cartItems) {
  ...
  // For site-wide percent codes with no min spend, allow discount even if breakdown is empty
  const isSitePercentNoMin = loaded.code.scope === "site" && loaded.code.type === "percent" && !loaded.code.min_spend_cents;
  if (!quote.discountCents && !isSitePercentNoMin) {
    return { ok: false, reason: "This cart does not meet the promo rules.", debug: quote.debug };
  }
...
export async function quoteBookingCode(raw, user, booking) {
  ...
  const isSitePercentNoMin = loaded.code.scope === "site" && loaded.code.type === "percent" && !loaded.code.min_spend_cents;
  if (!quote.discountCents && !isSitePercentNoMin) {
    return { ok: false, reason: "This booking does not meet the promo rules.", debug: quote.debug };
  }
  return { ok: true, code: loaded.code, quote };
}
