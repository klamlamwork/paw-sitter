...
export function maxRedeemablePoints(orderCents) {
  const capCents = Math.floor((Math.max(0, Number(orderCents) || 0) * MAX_REDEEM_PCT) / 100);
  return Math.floor(capCents / POINT_CENTS);
}

export function clampRedeem(points, available, orderCents, opts = {}) {
  const want = Math.floor(Number(points) || 0);
  const capByPct = maxRedeemablePoints(orderCents);
  const capByMax = opts.maxDiscountCents != null ? Math.floor(Number(opts.maxDiscountCents) / POINT_CENTS) : capByPct;
  const capPoints = Math.max(0, Math.min(capByPct, capByMax));
  const use = Math.min(Math.max(want, 0), floor0(available), capPoints);
  if (use < MIN_REDEEM) {
    return {
      ok: false,
      points: 0,
      cents: 0,
      capPoints,
      reason: `This order allows at most ${capPoints} points (40% cap${opts.maxDiscountCents != null ? ", fee-limited" : ""}). Minimum to redeem is ${MIN_REDEEM}.`,
    };
  }
  return { ok: true, points: use, cents: centsFromPoints(use), capPoints };
}
...
function computeBalance(rows = []) {
  const pending = floor0(rows.filter((r) => r.status === "pending").reduce((sum, r) => sum + (Number(r.delta) || 0), 0));
  const reserved = floor0(
    -rows.filter((r) => r.status === "reserved" && r.delta < 0).reduce((sum, r) => sum + (Number(r.delta) || 0), 0)
  );
  const spentByLot = {};
  for (const row of rows) {
    if (!row.lot_id || !(Number(row.delta) < 0)) continue;
    if (row.reason === "activate") continue;
    spentByLot[row.lot_id] = (spentByLot[row.lot_id] || 0) + Math.abs(Number(row.delta) || 0);
  }
  let available = 0;
  for (const lot of rows) {
    if (lot.status !== "available" || !(Number(lot.delta) > 0)) continue;
    available += Math.max(0, Number(lot.delta) - (spentByLot[lot.id] || 0));
  }
  return { available: floor0(available), pending, reserved };
}

async function loadLedger(admin, userId) {
  const { data, error } = await admin
    .from("paw_point_ledger")
    .select("id, user_id, delta, status, reason, source_key, lot_id, order_id, booking_id, expires_at")
    .eq("user_id", userId);
  if (error) throw error;
  return data || [];
}

async function repairMissingAvailableLots(admin, userId, rows) {
  const activateLotIds = new Set(rows.filter((r) => r.reason === "activate" && r.lot_id).map((r) => r.lot_id));
  const availableLotIds = new Set(
    rows.filter((r) => r.status === "available" && r.delta > 0 && r.lot_id).map((r) => r.lot_id)
  );
  let repaired = false;
  for (const lot of rows) {
    if (lot.status !== "pending" || !(lot.delta > 0)) continue;
    if (!activateLotIds.has(lot.id) || availableLotIds.has(lot.id)) continue;
    await appendLedger({
      user_id: userId,
      delta: lot.delta,
      status: "available",
      reason: lot.reason,
      source_key: lot.source_key || null,
      lot_id: lot.id,
      order_id: lot.order_id || null,
      booking_id: lot.booking_id || null,
...
export async function getBalance(userId) {
  const admin = createAdminClient();
  let rows = await loadLedger(admin, userId);
  if (await repairMissingAvailableLots(admin, userId, rows)) {
    rows = await loadLedger(admin, userId);
  }
...
async function existingRedeem({ userId, orderId, bookingId }) {
  const admin = createAdminClient();
  let q = admin.from("paw_point_ledger").select("delta").eq("user_id", userId).eq("reason", "redeem").lt("delta", 0);
  if (orderId) q = q.eq("order_id", orderId);
  else if (bookingId) q = q.eq("booking_id", bookingId);
  else return { points: 0, cents: 0, lots: [] };
  const { data, error } = await q;
  if (error) throw error;
  const points = (data || []).reduce((sum, row) => sum + Math.abs(row.delta), 0);
  return { points, cents: centsFromPoints(points), lots: [] };
}
...
export async function grantPendingEarn({ userId, points, reason, sourceKey, orderId, bookingId, remark }) {
  ...
  if (orderId || bookingId) {
    let q = admin.from("paw_point_ledger").select("id").eq("user_id", userId).eq("reason", reason).eq("status", "pending").gt("delta", 0);
    if (orderId) q = q.eq("order_id", orderId);
...
export async function activatePendingFor({ orderId, bookingId }) {
  const admin = createAdminClient();
  let q = admin.from("paw_point_ledger").select("id, user_id, delta, expires_at, source_key, reason, order_id, booking_id").eq("status", "pending").gt("delta", 0);
  if (orderId) q = q.eq("order_id", orderId);
  else if (bookingId) q = q.eq("booking_id", bookingId);
  else return [];
  ...
  const done = new Set();
  if (lotIds.length) {
    const { data: existingAvail } = await admin.from("paw_point_ledger").select("lot_id").eq("status", "available").gt("delta", 0).in("lot_id", lotIds);
    for (const row of existingAvail || []) if (row.lot_id) done.add(row.lot_id);
  }
  const out = [];
  for (const row of rows || []) {
    ...
    const avail = await appendLedger({
      ...
      booking_id: bookingId || row.booking_id || null,
      expires_at: row.expires_at,
      remark: "Available after fulfillment",
    });
    const { data: alreadyOffset } = await admin.from("paw_point_ledger").select("id").eq("reason", "activate").eq("lot_id", row.id).maybeSingle();
    if (!alreadyOffset) {
      await appendLedger({
        user_id: row.user_id,
        delta: -row.delta,
...
export async function reserveRedeem({ userId, points, orderId, bookingId }) {
  const already = await existingRedeem({ userId, orderId, bookingId });
  if (already.points > 0) return already;

  const balance = await getBalance(userId);
  const want = Math.min(floor0(points), balance.available);
...
export async function finalizeReserved({ orderId, bookingId }) {
  const admin = createAdminClient();
  let q = admin.from("paw_point_ledger").update({ remark: "Redeem posted" }).eq("status", "reserved").eq("reason", "redeem");
  if (orderId) q = q.eq("order_id", orderId);
  if (bookingId) q = q.eq("booking_id", bookingId);
  const { error } = await q;
  if (error) throw error;
