...
export async function getBalance(userId) {
  ...
  return {
    available: floor0(row?.available),
    pending: floor0(row?.pending),
    reserved: floor0(row?.reserved),
  };
}
...
export async function activatePendingFor({ orderId, bookingId }) {
  ...
  const done = new Set();
  if (lotIds.length) {
    const { data: activated } = await admin.from("paw_point_ledger").select("lot_id").eq("reason", "activate").in("lot_id", lotIds);
    for (const row of activated || []) if (row.lot_id) done.add(row.lot_id);
  }
  const out = [];
