...
export const PROFILE_POINTS = {
  basic: { first: 250, update: 100, cooldown: 0, reasonMin: 0 },
  diet: { first: 400, update: 100, cooldown: 30, reasonMin: 0 },
  hygiene: { first: 400, update: 100, cooldown: 30, reasonMin: 0 },
  medical: { first: 400, update: 100, cooldown: 60, reasonMin: 0 },
...
export function basicComplete(pet) {
  return filled(pet?.name) && filled(pet?.species) && (filled(pet?.birthday_year) || filled(pet?.age_years) || filled(pet?.age_months)) && filled(pet?.weight_lbs);
}
...
export function moduleProgress(pet, diet, hygiene, medical, social, episodes) {
  ...
  return {
    basic: basicComplete(pet) ? 100 : 50,
    diet: pct(dietNeed),
    hygiene: pct(hygNeed),
    medical: pct(medNeed),
    social: pct(socNeed),
...
export async function logProfileChange({ petId, userId, module, before, after, review, points }) {
  const admin = createAdminClient();
  const fields = changedFields(before, after);
  if (!fields.length && !points) return;
  await admin.from("pet_profile_history").insert({
    pet_id: petId,
    user_id: userId,
    module,
...
async function monthRows(admin, userId) {
...
export async function awardProfilePoints({ userId, petId, module, kind, reason = "", longevity = false }) {
  ...
  const rows = await monthRows(admin, userId);
  ...
  let bonus = 0;
  if (longevity && (module === "diet" || module === "hygiene")) {
...
export function basicChangedForPoints(before, after) {
  const keys = ["weight_lbs", "weight_unit", "sex", "breed", "is_spayed_neutered", "medications", "notes", "photo_url", "microchipped", "microchip_number", "vet_clinic"];
