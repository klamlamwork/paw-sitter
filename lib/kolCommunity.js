...
export async function productTaxonomySnapshot(admin, productId) {
...
export async function linkCommunityProducts(admin, postId, products, extrasById = {}) {
  ...
  for (let index = 0; index < products.length; index += 1) {
    ...
    const tax = await productTaxonomySnapshot(admin, product.id);
    rows.push({
      post_id: postId,
  ...
  const retry = await admin.from("shop_kol_post_products").insert(fallback);
