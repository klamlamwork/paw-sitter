...
export async function loadUserCartItems(supabase, userId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, product_id, variant_id, shop_id, qty, price_cents, currency, product:shop_products(id, name, slug), variant:shop_product_variants(id, name), shop:shop_shops(id, name, slug)"
    )
    .eq("cart_id", cartId);
  if (error) throw error;
  return (data || []).map((row) => ({
