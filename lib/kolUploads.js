...
import { KOL_LIMITS, KOL_ORPHAN_HOURS, KOL_UPLOAD_TTL_MINUTES, KOL_COMMUNITY_VIDEO_MAX_SECONDS } from "@/lib/kolPolicy";
...
async function assertOwnedActiveDraft(admin, { userId, postId }) {
  ...
  const { data: post } = await admin.from("shop_kol_posts").select("id, author_profile_id, source_type, status").eq("id", postId).maybeSingle();
...
export async function createKolUploadSession({ userId, mediaKind, postId }) {
  ...
  const { count: hourly } = await admin.from("kol_upload_sessions").select("id", { count: "exact", head: true }).eq("profile_id", userId).gte("created_at", since);
  if ((hourly || 0) >= 20) throw new Error("Upload limit reached. Try again later.");
  const maxImages = post.source_type === "community" ? 40 : KOL_LIMITS.maxImages;
  const { count: images } = await admin.from("kol_upload_sessions").select("id", { count: "exact", head: true }).eq("post_id", post.id).eq("media_kind", "image").in("status", ["open", "uploaded", "attached"]);
  const { count: videos } = await admin.from("kol_upload_sessions").select("id", { count: "exact", head: true }).eq("post_id", post.id).eq("media_kind", "video").in("status", ["open", "uploaded", "attached"]);
  if (kind === "image" && (images || 0) >= maxImages) throw new Error(post.source_type === "community" ? "This post already has the maximum photos." : "This post already has the maximum 10 images.");
  ...
  const videoMax = post.source_type === "community" ? KOL_COMMUNITY_VIDEO_MAX_SECONDS : KOL_LIMITS.videoMaxSeconds;
  const row = {
    id: sessionId,
    profile_id: userId,
    post_id: post.id,
    ...
    max_duration_seconds: kind === "video" ? videoMax : null,
...
export async function markKolUploaded(sessionId, extra = {}) {
  const admin = createAdminClient();
  const { data: session } = await admin.from("kol_upload_sessions").select("id, post_id, profile_id, folder, public_id, resource_type, max_bytes, max_duration_seconds, status").eq("id", sessionId).maybeSingle();
  ...
  if (session.resource_type === "video" && extra.duration_seconds && Number(extra.duration_seconds) > Number(session.max_duration_seconds || 0)) {
    throw new Error(Number(session.max_duration_seconds) >= 900 ? "Uploaded video exceeds the 15-minute limit." : "Uploaded video exceeds the 90-second limit.");
  }
  ...
  const { data, error } = await admin.from("shop_kol_post_media").insert({
  ...
  }).select("id, public_id, version, resource_type").single();
  if (error) throw error;
  return { id: data.id, post_id: session.post_id, public_id: data.public_id, version: data.version, resource_type: data.resource_type };
}

export async function cleanupOrphanKolUploads() {
  const admin = createAdminClient();
  const cutoff = new Date(Date.now() - KOL_ORPHAN_HOURS * 3600 * 1000).toISOString();
  const { data: sessions } = await admin.from("kol_upload_sessions").select("id, public_id, folder, resource_type").in("status", ["open", "uploaded"]).lt("created_at", cutoff);
  const deleted = [];
